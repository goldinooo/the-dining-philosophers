...
void philos(t_philo *philo, t_philo_data *program, pthread_mutex_t *forks, char **argv)
{
	int i;
	...
	while (i < philo->num_philos)
	{
		...
		philo[i].start_time = get_microtime();
		philo[i].last_meal = get_microtime();
		...
		if (i == 0) // first philosopher
			philo[i].r_fork = &forks[philo->num_philos - 1];
		else
